...
function prb58(){
  var solution = 0,
      num = 0,
      den = 1;
  for(var x = 3; x < 99999; x=x+2){
    den += 4;
    // Math.pow(x,2) is not a prime so...
    if(isprime(Math.pow(x,2)-x+1)) num++;
    if(isprime(Math.pow(x,2)-(2*x)+2)) num++;
    if(isprime(Math.pow(x,2)-(3*x)+3)) num++;
    if((num/den*100)<10){
      solution = x;
      break;
    }
